	...
	for _, b := range f.Blocks {
		...
		for idx, v := range oldSched {
				...
				if mask&wreg != 0 {
					...
					if len(w.Args) == 0 {
						...
						if w.Op == OpSB {
							...
							c = b.NewValue1(w.N, OpCopy, w.Type, w)
						} else {
							c = b.NewValue0IA(w.N, w.Op, w.Type, w.AuxInt, w.Aux)
						}
					} else if len(w.Args) == 1 && (w.Args[0].Op == OpSP || w.Args[0].Op == OpSB) {
						// Materialize offsets from SP/SB
						c = b.NewValue1IA(w.N, w.Op, w.Type, w.AuxInt, w.Aux, w.Args[0])
					} else if wreg != 0 {
						...
						s := pickReg(wreg)
						// inv: s != r
						c = b.NewValue1(w.N, OpCopy, w.Type, regs[s].c)
					} else {
						// Load from home location
						c = b.NewValue1(w.N, OpLoadReg8, w.Type, w)
					}
			...
			if len(outputs) == 0 || outputs[0] == 0 {
				...
				c := b.NewValue0IA(v.N, v.Op, v.Type, v.AuxInt, v.Aux)
				c.AddArgs(v.Args...)
				home = setloc(home, c, &registers[r])
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			for i, w := range v.Args {
				c := b.Preds[i]
				cpy := c.NewValue1(w.N, OpCopy, v.Type, w)
				v.Args[i] = cpy
			}
		}
	}
