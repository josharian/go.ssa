...
func rewriteValueAMD64(v *Value, config *Config) bool {
	...
	case OpConvNop:
		...
		// result: (Copy x)
		{
			...
			x := v.Args[0]
			if !(t.Equal(x.Type())) {
				goto enda57c92feeea399b5dd95c546af3d6e47
			}
			v.Op = OpCopy
			v.AuxInt = 0
			v.Aux = nil
		...
		goto enda57c92feeea399b5dd95c546af3d6e47
	enda57c92feeea399b5dd95c546af3d6e47:
		;
		// match: (ConvNop <t> x)
		...
		// result: (Copy x)
		{
			...
			if !(t.IsInteger() && x.Type().IsInteger() && t.Size() == x.Type().Size()) {
				goto end282b6cab007cafab86a75df485522973
			}
			v.Op = OpCopy
			v.AuxInt = 0
			v.Aux = nil
		...
		goto end282b6cab007cafab86a75df485522973
	end282b6cab007cafab86a75df485522973:
		;
	case OpEq16:
		// match: (Eq16 x y)
		// cond:
