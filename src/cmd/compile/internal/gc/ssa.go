...
var skipfn = map[pkgfn]bool{
	...
	pkgfn{pkg: "zip", fn: "(*FileHeader).isZip64"}:           true,
	pkgfn{pkg: "flate", fn: "(*huffmanBitWriter).writeCode"}: true,
	pkgfn{pkg: "elliptic", fn: "p224Reduce"}:                 true,
	pkgfn{pkg: "x509", fn: "asn1BitLength"}:                  true,
	pkgfn{pkg: "sql", fn: "(*fakeConn).isBad"}:               true,
	pkgfn{pkg: "fmt", fn: "(*pp).fmt0x64"}:                   true,
	pkgfn{pkg: "constant", fn: "is63bit"}:                    true,
	pkgfn{pkg: "printer", fn: "mayCombine"}:                  true,
	pkgfn{pkg: "token", fn: "Token.IsOperator"}:              true,
	pkgfn{pkg: "types", fn: "TypeAndValue.HasOk"}:            true,
	pkgfn{pkg: "image", fn: "(*YCbCr).COffset"}:              true,
	pkgfn{pkg: "jpeg", fn: "fdct"}:                           true,
	pkgfn{pkg: "big", fn: "(*Float).IsInt"}:                  true,
	pkgfn{pkg: "net", fn: "(*conn).ok"}:                      true,
	pkgfn{pkg: "http", fn: "isNotToken"}:                     true,
	pkgfn{pkg: "mail", fn: "isWSP"}:                          true,
	pkgfn{pkg: "runtime", fn: "strequal"}:                    true,
	pkgfn{pkg: "runtime", fn: "strequal"}:                    true,
	pkgfn{pkg: "zip", fn: "fileModeToUnixMode"}:              true,
	pkgfn{pkg: "token", fn: "Token.IsKeyword"}:               true,
	pkgfn{pkg: "types", fn: "isShift"}:                       true,
	pkgfn{pkg: "types", fn: "isShift"}:                       true,
	pkgfn{pkg: "big", fn: "(*Float).SetInt64"}:               true,
	pkgfn{pkg: "net", fn: "(*UnixAddr).isWildcard"}:          true,
